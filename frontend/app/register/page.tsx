...
import { login, register, saveToken, ApiError } from "@/lib/api";
...
function OAuthButton({ icon, label }: { icon: React.ReactNode; label: string }) {
  ...
  return (
    <div className="relative flex-1">
      <button
        type="button"
        onClick={() => { setTip(true); setTimeout(() => setTip(false), 2500); }}
        className="group flex h-11 w-full items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[0.03] text-sm font-medium text-zinc-300 transition-all hover:border-white/20 hover:bg-white/[0.06] hover:text-zinc-50 active:scale-[0.98]"
      >
        {icon}
        {label}
      </button>
      {tip && (
        <div className="absolute -top-9 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-lg border border-white/10 bg-zinc-900 px-3 py-1.5 text-[11px] text-zinc-400 shadow-lg">
          Coming soon
        </div>
      )}
    </div>
